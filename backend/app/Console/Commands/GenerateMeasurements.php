...
{
    /**
     * Define the console command signature.
     */
    ...
    public function handle()
    {
        ...
        $bufferSize = (int) $this->argument('batch-size');

        if ($count <= 0) {
            $this->error("Please provide a valid number of records.");
            return;
        }

        $filePath = $this->argument('path') . '/measurements.txt' ?? storage_path('app/measurements.txt');
    ...
    private function generateTemperature(float $mean): float
    {
        $stdDev = 10.0;
